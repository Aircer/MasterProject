...
namespace MapTileGridCreator.Paint
{
	public static class IndexesToPaint
	{
		...
		public static HashSet<Vector3Int> AddInput(Vector3Int _size_grid, Vector3Int input, PaintMode _mode_paint, CellInformation _cellType,
										ref Cell[,,] _cells, ref WaypointCluster _cluster)
		{
			...
			{
				newIndexToPaint = CellsPainted(input, _startingPaintIndex, _mode_paint, _cells, _cellType);
				...
				foreach (Vector3Int newIndex in newIndexToPaint)
				{
					...
					if (!indexExist)
					{
						if (_mode_paint == PaintMode.Single)
						{
							...
							FuncMain.PaintCell(_cells, newIndex, _cellType);
							_cluster.SetTypeAndRotationAround(_size_grid, _cellType, newIndex);
						}
						else if (_mode_paint == PaintMode.Erase && _cluster.GetWaypoints()[newIndex.x, newIndex.y, newIndex.z].type != null)
						{
							_indexToPaint.Add(newIndex);
							FuncMain.EraseCell(_cells, _cluster, newIndex);
						}

						FuncVisual.UpdateCellsAroundVisual(_cells, _cluster.GetWaypoints(), newIndex, _cellType);
					}
				}
				...
				foreach (Vector3Int currentIndex in _indexToPaint)
				{
					...
					if (indexNoMorePainted)
					{
						if (_mode_paint == PaintMode.Single)
						{
							FuncMain.DesactivateCell(_cells, _cluster, currentIndex);
