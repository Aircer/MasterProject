function cells = GetCells(IDExperiment, candidatesNumber, numberRuns, generations, population, size)
...
for i=1:candidatesNumber
    ...
    cellsRun = zeros((generations + 1)*population, size(1)*size(2)*size(3));
...
%         [~,~,data] = xlsread(strcat(pathCells, num2str(j - 1), '.csv'));
        ...
        for k = 1:generations+1
            cells(i, j, k, :, :) = cellsRun((k-1)*population + 1:k*population, :); 
        end
    end
end
end
